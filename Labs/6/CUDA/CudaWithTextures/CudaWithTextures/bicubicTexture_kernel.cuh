...
R tex2DBicubic(const texture<T, 2, cudaReadModeNormalizedFloat> texref, float x, float y)
{
	...
	//TODO: do more pixels to see a smoother image.
	R center = tex2D(texref, px, py);
	R left = tex2D(texref, px - 1, py);
	R right = tex2D(texref, px + 1, py);
	R up = tex2D(texref, px, py + 1);

	R down = tex2D(texref, px, py - 1);

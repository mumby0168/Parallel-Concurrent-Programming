...
void hostMultiply()
{
	...
	const int resultWidth = widthB;
	const int resultHeight = heightA;
	...
	const int matrixB[arraySizeB] = { 10, 5,
									  4, 3,
									  2, 5 };
	int result[arraySizeC] = { 0, 0,
							   0, 0,
							   0, 0,
							   0, 0 };

	// looping over the rows of A (4)
	for (int i = 0; i < widthA; i++)
	{		
		// looping over the columns of a
		for (int j = 0; j < widthA; j++)
		{	
			// looping over the height of b
			for (int k = 0; k < widthA; k++)
			{
				int aIndex = i * widthA + k; //Correct
				int bIndex = j + widthB * k;
				printf("b[%d] = %d\n", bIndex, matrixB[bIndex]);
